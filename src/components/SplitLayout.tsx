...
export function SplitLayout({
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#050508] text-slate-200 font-sans selection:bg-[#A89882]/30 overflow-hidden flex flex-col md:flex-row relative">
      {/* 全域頂部導航 */}
      <header className="absolute top-0 left-0 w-full px-5 pb-4 pt-[calc(var(--safe-top)+14px)] md:p-6 z-50 flex justify-between items-center pointer-events-none">
        <div
          className="flex items-center gap-3 cursor-pointer pointer-events-auto"
          onClick={() => nav('/')}
        >
          <h1 className="text-base md:text-lg font-light tracking-[0.34em] md:tracking-[0.4em] text-white drop-shadow-md flex items-center gap-2">
            MYSTIC
            <span className="hidden sm:inline text-[9px] text-white/50 tracking-widest mt-1">首頁 HOME</span>
          </h1>
        </div>
        <div className="flex items-center gap-4 pointer-events-auto">
          {showLogout && (
            <button
              onClick={() => void logout()}
              className="text-white/60 hover:text-[#A89882] tracking-[0.18em] md:tracking-[0.2em] text-[10px] transition-colors flex items-center gap-2 border border-white/10 px-3 md:px-4 py-2 rounded-full backdrop-blur-md bg-black/20"
            >
              登出 LOGOUT
            </button>
          )}
      ...
      <div className="w-full flex-1 min-h-0 md:h-full md:w-[55%] lg:w-[50%] overflow-y-auto z-20 md:order-1 relative custom-scrollbar">
        ...
        <div className="min-h-full flex flex-col justify-start md:justify-center p-5 md:p-12 lg:p-16 pt-4 md:pt-24 pb-[calc(var(--safe-bottom)+24px)] md:pb-20">
          {children}
        </div>
      </div>
    </div>
